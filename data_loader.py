...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # Load the CSV into Neo4j using LOAD CSV
        ...
        with self.driver.session(database="neo4j") as session:
            ...
            # Constraint to ensure uniqueness of Location.name
            session.run("""
            ...
            WITH row
            WHERE row.PULocationID IS NOT NULL
              AND row.DOLocationID IS NOT NULL
            ...
            session.run(cypher, csv_url=csv_url)
